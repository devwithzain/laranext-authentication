...
import { profileFormSchema, TprofileFormData } from "@/schemas";
...
export default function ProfileForm() {
	...
	const deleteAccount = async () => {
		await axios
			.delete(
				`${process.env.NEXT_PUBLIC_BACKEND_LARAVELA_URL}/profile/delete`,
				{
					headers: {
	...
	const onSubmits = async (data: TprofileFormData) => {
		await axios
			.patch(
				`${process.env.NEXT_PUBLIC_BACKEND_LARAVELA_URL}/profile/update`,
				data,
				{
					headers: {
						Authorization: `Bearer ${token}`,
			...
			<div className="w-full">
				<div className="w-full fixed top-0 px-10 py-5 flex items-center justify-between bg-gray-200">
					...
					<div>
						<button
							onClick={() => logOut()}
							className="text-lg bg-[#3A364D] px-4 py-2 rounded-lg text-white font-semibold leading-tight tracking-tight">
							Logout
						</button>
					</div>
				</div>
